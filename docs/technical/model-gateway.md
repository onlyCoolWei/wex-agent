...
| response usage            | `usage.updated`     | 只暴露 input/output token 数          |
...
- Langfuse observation 按当前明确授权记录 Agent 的 system prompt、完整用户消息正文和模型输出，以便排查语义问题；API key 仍不会进入 observation。输入以配置中的 system prompt 为首条消息，并复用 Runner 实际使用的 OpenAI Agents SDK `AgentInputItem[]`（包括 assistant 的 `status` 与嵌套 `content`），generation 额外记录模型别名、配置版本和 token usage。
